...
class OrphanRecordsService extends AbstractCleanupService
{
    ...
    protected function execute(
        ...
        if (count($orphans)) {
            ...
            if($dryRun) {
                $this->addMessage('Deletion process starting now.' . ($dryRun ? ' (Not deleting now, just a dry run)' : ''));
            } else {
                // Actually permanently delete them
            return $this->deleteRecords($orphans);
            }
        } else {
            $this->addMessage('No orphan records found.');
        }
    }
    ...
    protected function deleteRecords(array $orphanedRecords)
    {
        ...
        foreach ($orphanedRecords as $table => $list) {
            foreach ($list as $uid) {
                $this->addMessage('Flushing record "' . $table . ':' . $uid . '"');
